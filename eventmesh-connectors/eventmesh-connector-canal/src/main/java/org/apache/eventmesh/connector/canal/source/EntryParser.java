...
public class EntryParser {
     ...
     * </pre>
     */
    public List<ConnectRecord> parse(CanalSourceConfig sourceConfig, List<Entry> datas) throws SelectException {
        List<ConnectRecord> eventDatas = new ArrayList<ConnectRecord>();
...
//                                       && pipelineParameter.getRemedyAlgorithm().isLoopback();
        boolean enableLoopbackRemedy = false;
            ...
            for (Entry entry : datas) {
                switch (entry.getEntryType()) {
                    ...
                    case ROWDATA:
                        ...
                        // 判断是否是回环表retl_mark
                        boolean isMarkTable = tableName.equalsIgnoreCase(sourceConfig.getSystemMarkTable());
                        if (isMarkTable) {
                            RowChange rowChange = RowChange.parseFrom(entry.getStoreValue());
                            if (!rowChange.getIsDdl()) {
                                int loopback = 0;
                                if (rowChange.getRowDatasCount() > 0) {
                                    loopback = checkLoopback(sourceConfig, rowChange.getRowDatas(0));
                                }
                                if (loopback == 2) {
                        ...
                        if (isCompatibleLoopback) {
                            ...
                            if (!rowChange.getIsDdl()) {
                                int loopback = 0;
                                if (rowChange.getRowDatasCount() > 0) {
                                    loopback = checkCompatibleLoopback(sourceConfig, rowChange.getRowDatas(0));
                                }
                                if (loopback == 2) {
                                    needLoopback = true; // 只处理正常同步产生的回环数据
                                }
                        ...
                        if (!isLoopback && !isMarkTable && !isCompatibleLoopback) {
                    ...
                    case TRANSACTIONEND:
                        if (!isLoopback || (enableLoopbackRemedy && needLoopback)) {
                            ...
                            for (Entry bufferEntry : transactionDataBuffer) {
                                List<ConnectRecord> parseDatas = internParse(sourceConfig, bufferEntry);
                                ...
                                for (ConnectRecord record : parseDatas) {
                                    if (record == null) {
                                        continue;
                                    }
     ...
     * </pre>
     */
    private int checkLoopback(CanalSourceConfig sourceConfig, RowData rowData) {
        // 检查channel_info字段
        // 首先检查下after记录，从无变有的过程，一般出现在事务头
        Column infokColumn = getColumnIgnoreCase(rowData.getAfterColumnsList(), sourceConfig.getSystemMarkTableInfo());
        ...
        infokColumn = getColumnIgnoreCase(rowData.getBeforeColumnsList(), sourceConfig.getSystemMarkTableInfo());
        // 匹配对应的channelInfo，如果以_SYNC结尾，则认为需要忽略
        if (infokColumn != null && StringUtils.endsWithIgnoreCase(infokColumn.getValue(), RETL_CLIENT_FLAG)) {
            return 1;
        }
